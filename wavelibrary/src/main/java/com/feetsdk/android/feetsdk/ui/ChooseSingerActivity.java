...
//            System.out.println("Key = " + entry.getKey() + ", Value = " + entry.getValue());
    ...
    private void updateSinger(){
        if (chooseThree()) {
            httpControler.postArtists(
                    chooseSingerList.get(1).getId(),
                    chooseSingerList.get(2).getId(),
                    chooseSingerList.get(3).getId(), new IHttpRspCallBack() {
                        ...
                        @Override
                        public void failed(HttpException exception) {
                            ToastUtil.showToast(ChooseSingerActivity.this,"重置歌手失败，请检查网络连接");
                        }
                    });
        }else{
            ToastUtil.showToast(this,"请选择三位歌手");
        }
    }
